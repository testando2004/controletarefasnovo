...
export default function ModalNovaEmpresa({ onClose }: ModalNovaEmpresaProps) {
  ...
  const [projetosDisponiveis, setProjetosDisponiveis] = useState<Array<{ id: number; nome: string }>>([]);
  ...
  // Carrega projetos da empresa selecionada (ativos)
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setProjetosDisponiveis(ativos.map((p: any) => ({ id: p.id, nome: p.nome })));
      } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Gerente só pode criar solicitações para um dos seus departamentos (principal ou extras)
    ...
    try {
      ...
      // Se usuário pediu novo projeto, cria primeiro
      let projetoIdFinal: number | null = typeof projetoId === 'number' ? projetoId : null;
      if (mostrarNovoProjeto && novoProjetoNome.trim()) {
        try {
          const novo = await api.criarProjeto({
            nome: novoProjetoNome.trim(),
            empresaId: empresaSelecionada.id,
            responsavelId: typeof responsavelId === 'number' ? responsavelId : null,
          });
          projetoIdFinal = Number(novo?.id) || null;
        } catch (err: any) {
        ...
        <form onSubmit={handleSubmit} className="space-y-6 p-4 sm:p-6" onKeyDown={(e) => { if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 's') { e.preventDefault(); (e.currentTarget as HTMLFormElement).requestSubmit(); } }}>
          ...
          <div className="bg-cyan-50 dark:bg-[#0f2b34] rounded-xl p-4 border border-cyan-200 dark:border-[#155e75]">
            ...
            {empresaSelecionada && (
              <div className="mt-4">
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  📁 Projeto <span className="text-gray-400 text-xs font-normal">(opcional — agrupa solicitações do mesmo assunto)</span>
                </label>
                {!mostrarNovoProjeto ? (
                  ...
                  <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-700 rounded-xl p-3 space-y-2">
                    <input
                      ...
                      placeholder={`Ex.: Abertura de Empresa — ${empresaSelecionada.razao_social || empresaSelecionada.apelido || 'Empresa'}`}
                ...
                {typeof projetoId === 'number' && projetoId > 0 && (
                  <p className="text-xs text-indigo-600 dark:text-indigo-400 mt-1">
                    ✓ Esta solicitação entra no dossiê. Interligações automáticas herdam o projeto.
                  </p>
                )}
              </div>
            )}

...
interface SistemaContextType {
  ...
  criarTemplate: (dados: {
    ...
    interligacaoParalelo?: boolean;
    interligacaoGrupos?: Array<{ modo: 'sequencial' | 'paralelo'; templateIds: number[] }>;
    dependenciasDept?: Record<string, string[]>;
  }) => Promise<Template>;
  atualizarTemplate: (templateId: number, dados: {
    nome?: string;
    descricao?: string;
    fluxoDepartamentos?: number[];
    questionariosPorDepartamento?: any;
  ...
  finalizarProcesso: (processoId: number) => Promise<{ finalizado: boolean; processoId: number; interligadoComId: number | null; interligadoParalelo: boolean; processoNome: string; interligacaoTemplateIds: number[]; interligacaoGrupos: Array<{ modo: 'sequencial' | 'paralelo'; templateIds: number[] }>; projetoId: number | null } | void>;
  ...
  voltarParaDepartamentoAnterior: (processoId: number, destinoDepartamentoId?: number) => Promise<void>;
 ...
 .on('postgres_changes', { event: '*', schema: 'public', table: 'Processo' }, (payload) => {
    ...
    async (dados: {
      ...
      try {
        const novo = await api.salvarTemplate({
          ...
          interligacaoTemplateIds: Array.isArray(dados.interligacaoTemplateIds) ? dados.interligacaoTemplateIds : [],
          interligacaoParalelo: Boolean(dados.interligacaoParalelo),
          interligacaoGrupos: Array.isArray(dados.interligacaoGrupos) ? dados.interligacaoGrupos : [],
          dependenciasDept: dados.dependenciasDept && typeof dados.dependenciasDept === 'object' ? dados.dependenciasDept : {},
        });
  ...
  const atualizarTemplate = useCallback(
    async (
      ...
      try {
        const atualizado = await api.atualizarTemplate(templateId, {
          nome: dados.nome,
          descricao: dados.descricao,
          fluxoDepartamentos: dados.fluxoDepartamentos,
          questionariosPorDepartamento: dados.questionariosPorDepartamento,
        ...
        setTemplates(prev => prev.map(t => (t.id === templateId ? atualizado : t)));
        adicionarNotificacao('Template atualizado com sucesso', 'sucesso');
        api.registrarLog?.({ acao: 'ATUALIZAR', entidade: 'TEMPLATE', entidadeId: templateId, entidadeNome: atualizado?.nome });
        return atualizado;
      } catch (error: any) {
        adicionarNotificacao(error.message || 'Erro ao atualizar template', 'erro');
        throw error;
      }
    },
    [adicionarNotificacao]
    ...
    async (dados: Partial<Processo>) => {
      try {
        ...
        const novo = await api.salvarProcesso({
          ...
          projetoId: (dados as any).projetoId,
          interligacaoGrupos: (dados as any).interligacaoGrupos,
          dependenciasDept: (dados as any).dependenciasDept,
        } as any);
    ...
    try {
      ...
      return {
        ...
        interligacaoTemplateIds: Array.isArray((processoCompleto as any).interligacaoTemplateIds)
          ...
          : [],
        interligacaoGrupos: Array.isArray((processoCompleto as any).interligacaoGrupos)
          ? ((processoCompleto as any).interligacaoGrupos as any[])
              .map((g: any) => ({
                modo: (g?.modo === 'paralelo' ? 'paralelo' : 'sequencial') as 'sequencial' | 'paralelo',
                templateIds: Array.isArray(g?.templateIds)
                  ? g.templateIds.map(Number).filter((n: any) => Number.isFinite(n) && n > 0)
                  : [] as number[],
              }))
              .filter((g: any) => g.templateIds.length > 0)
          : ([] as Array<{ modo: 'sequencial' | 'paralelo'; templateIds: number[] }>),
        projetoId: Number.isFinite(Number((processoCompleto as any).projetoId))
          ? Number((processoCompleto as any).projetoId)
          : null,
      };
    } catch (error: any) {
  ...
  const voltarParaDepartamentoAnterior = useCallback(async (processoId: number, destinoDepartamentoId?: number) => {
    try {
      const processo = processos.find(p => p.id === processoId);
      const deptDestino = destinoDepartamentoId ? departamentos.find(d => d.id === destinoDepartamentoId) : null;
      const confirmado = await mostrarConfirmacao({
        titulo: 'Confirmar retorno',
        mensagem: deptDestino
          ? `Deseja retornar esta solicitação para o departamento "${deptDestino.nome}"? O fluxo seguirá normalmente a partir dele.`
          : 'Deseja retornar este processo ao departamento anterior?',
        tipo: 'aviso',
      ...
      await api.voltarProcesso(processoId, destinoDepartamentoId);
      const processoAtualizado = await api.getProcesso(processoId);
      setProcessos(prev => prev.map(p => p.id === processoId ? processoAtualizado : p));
      adicionarNotificacao(
        deptDestino
          ? `Solicitação retornada para "${deptDestino.nome}"`
          : 'Processo retornado ao departamento anterior',
        'sucesso'
      );
